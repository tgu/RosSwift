...
internal final class ServiceServerLink: ChannelInboundHandler, @unchecked Sendable {
    var channel: Channel?
    ...
    func call(req: SerializedMessage) -> EventLoopFuture<SerializedMessage> {
        ...
        guard let c = channel else {
            let promise: EventLoopPromise<SerializedMessage> = threadGroup.next().makePromise()
            promise.fail(ServiceError.invalidInput("ServiceServerLink::call has no connection"))
            return promise.futureResult
        }
        let promise: EventLoopPromise<SerializedMessage> = c.eventLoop.makePromise()
        ...
        while buffer.readableBytes > 0 {
            ...
            case .header:
                guard let len: UInt32 = buffer.readInteger(endianness: .little) else {
                    ROS_ERROR("Received an invalid TCPROS header. Invalid count")
                    _ = context.close()
                    return
                }
                guard Int(len) <= buffer.readableBytes else {
                    ROS_ERROR("Received an invalid TCPROS header. Length \(len) exceeds buffer size \(buffer.readableBytes)")
                    _ = context.close()
                    return
                }
                ...
                while buffer.readableBytes > leave {
                    guard let topicLen: UInt32 = buffer.readInteger(endianness: .little) else {
                        ROS_ERROR("Received an invalid TCPROS header. Invalid string")
                        _ = context.close()
                        return
                    }

                    guard let line = buffer.readString(length: Int(topicLen)) else {
                        ROS_ERROR("Received an invalid TCPROS header. Could not read string of length \(topicLen)")
                        _ = context.close()
                        return
                    }

                    guard let equalIndex = line.firstIndex(of: "=") else {
                        ROS_ERROR("Received an invalid TCPROS header. Each line must have an equals sign.")
                        _ = context.close()
                        return
                    }
            ...
            case .message:
                guard let ok: UInt8 = buffer.readInteger(endianness: .little) else {
                    ROS_ERROR("Received malformed message")
                    _ = context.close()
                    return
                }
                ROS_DEBUG("OK = \(ok)")
