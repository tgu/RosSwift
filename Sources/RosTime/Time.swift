...
public struct Time: TimeBase, Sendable {
    public typealias Duration = RosDuration
    ...
    public static let simTime = Mutex(Time())
    public static var max: Time { Time(nanosec: UInt64(Int64.max)) }
    public static var min: Time { Time(nanosec: 1) }
    ...
    public static var isValid: Bool {
        return !useSimTime.load(ordering: .relaxed) || !Time.simTime.withLock { $0.isZero }
    }
    ...
    public static func waitForValid(timeout: WallDuration = WallDuration()) async -> Bool {
        let start = WallTime.now
        while !isValid && !gStopped.load(ordering: .relaxed) {
            _ = await WallDuration(seconds: 0.01).sleep()
